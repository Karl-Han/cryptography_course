...
impl Buffer{
    ...
    pub fn keccak(&mut self, round : u8){
        ...
        for i in 0..round {
            ...
            // RHO operation
            for i in 1..25 {
                self.buf[i] = self.buf[i].rotate_right(RHO[i -1] % 64);
            }
            ...
            for x in 0..5 {
                for y in 0..5 {
                    temp_state[5 * y] = self.buf[5 * x + ((x + 3 * y) %5 ) ]
                }
            }
            self.buf = temp_state;
...
impl KeccakState{
    ...
    pub fn h2s(buf : [u8; 200]) -> [u64; 24]{
        ...
        for i in 0..24 {
            array[i] = u64::from_le_bytes(buf[i * 8..(i + 1) *8].try_into().expect("h2s: [u8] to u64"));
        }
    ...
    pub fn update(&mut self, reader : &mut dyn Read){
        let mut buf = [0u8; 200];
        let bytes_read = reader.read(&mut buf).expect("Failed to read");
        dbg!(bytes_read);
        //if bytes_read != 0{
            if bytes_read < 200{
...
pub trait Hasher {
    fn hash_file(&mut self, filename : String, buf :&mut [u8]);
    fn hash_str(&mut self,s : &str, buf : &mut [u8]);
}
